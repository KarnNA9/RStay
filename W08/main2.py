    ...
    with torch.no_grad():
        ...
        for _ in range(10):
            ...
            if pred_token == output_word2idx['<EOS>']:
                break
        trg_tokens = [output_vocab[idx] for idx in trg_indexes if idx in output_word2idx.values()]
        # trg_tokens = [list(output_vocab)[idx] for idx in trg_indexes if idx in output_word2idx.values()]
        return trg_tokens[1:]
